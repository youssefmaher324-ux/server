...
let ACCESS_KEY = sessionStorage.getItem('citrine_admin_key') || '';
let ORDERS = [];
let PRODUCTS = [];
let DRIVERS = [];
let pollTimer = null;
...
async function tryUnlock() {
  ...
  try {
    ...
    if (who.role !== 'admin') throw new Error('This key is not an admin key.');
    sessionStorage.setItem('citrine_admin_key', key);
    enterDashboard();
  } catch (err) {
...
document.getElementById('logoutBtn').onclick = () => {
  sessionStorage.removeItem('citrine_admin_key');
